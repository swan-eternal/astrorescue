...
> **Status (2026-07-09):** Bug fixes shipped (2 commits): menu centering (`scripts/pause_menu.gd`, `scripts/settings_menu.gd` — replaced `PRESET_CENTER` with `CenterContainer` wrapping, since `PRESET_CENTER` keeps offsets at 0/0/0/0 on a zero-size Control and renders the panel at origin instead of center); settings panel opacity (`scripts/settings_menu.gd` — per-instance `StyleBoxFlat` override with alpha=1.0; Godot's default `PanelContainer` stylebox is 75% opaque and let menu text bleed through); escape trajectory shows the full analytical hyperbola (`scripts/trajectoryline_2d.gd` — removed the world-space `max_leg_radius` clip in `_hyperbola_leg` so the curve extends to its natural asymptote; added radial-escape fallback for h=0 launches). **SOI visualization shipped**: shaded circle per planet via the new `scripts/soi_indicator.gd`, both in the level editor (always visible — `bypass_visual_settings = true` so the planning aid stays on regardless of the user's in-game setting) and in-game (toggleable via Settings → Visual → "Show Planet SOI", default off). Hill-sphere math centralized in `OrbitCalculator.compute_soi_radius` (and `DEFAULT_SOI_FRACTION = 0.5` constant) so the trajectory-line detection and the editor indicator share one source of truth. New `VisualSettings` autoload (`scripts/visual_settings.gd`) mirrors `AudioSettings` — persists `show_soi` to `user://visual_settings.cfg` (separate file from audio, no coordination needed). Working tree (this batch): doc updates, Godot drive-bys (`AstroRescueV01.exe`, `export_presets.cfg`), new auto-generated `.uid` files for scripts that gained cross-script references (`audio_settings`, `audio_tab`, `gameplay_tab`, `settings_menu`, `visual_tab`).
>
> **Engine:** Godot 4.x
...
- `AudioManager` (`scripts/audio_manager.gd`) — music + SFX (menu + gameplay music, thruster loop, pickup SFX). Routed: `_music_player.bus = "Music"`, `_thruster_player.bus = "SFX"`, oneshots `bus = "SFX"`.
- `AudioSettings` (`scripts/audio_settings.gd`) — persistent per-bus volume + mute state (channels: master / music / sfx, each with `db` clamped to [-40, 0] + `muted` bool); saved to `user://settings.cfg`. Read by `scripts/audio_tab.gd`; pushes to `AudioServer.set_bus_volume_db` / `set_bus_mute` and persists immediately on each setter call. Loaded once in `_ready` and reapplied to AudioServer so persisted values survive restarts.
...
| `scripts/audio_manager.gd` | Music + SFX (autoload). Routed: `_music_player.bus = "Music"`, `_thruster_player.bus = "SFX"`, oneshots `bus = "SFX"`. |
| `scripts/audio_settings.gd` | Persistent volume + mute state for the three audio buses (autoload). ConfigFile at `user://settings.cfg`; channels `master`/`music`/`sfx`, each `db` + `muted`. |
| `scripts/audio_tab.gd` | Settings panel: 3 rows (Master/Music/SFX) with name + 0..100 slider (linear → -40..0 dB) + live dB readout + mute checkbox. Reads/writes `AudioSettings`. |
| `scripts/settings_menu.gd` | Tabbed settings panel (Audio/Visual/Gameplay). CanvasLayer with `PROCESS_MODE_ALWAYS`, layer 110. Full-rect backdrop with `mouse_filter = STOP`. Esc + Close button queue_free self. Static `_is_any_open` flag + `is_any_open()` accessor gates the pause menu's Esc handler (so Esc-closes-settings doesn't also resume the game). |
| `scripts/visual_tab.gd` | Settings panel placeholder (future: fullscreen, brightness). |
| `scripts/gameplay_tab.gd` | Settings panel placeholder (future: trajectory default, time-warp defaults). |
...
- ~~**Faster time warp** — extend `TIME_WARP_LEVELS` from `[1, 2, 4, 8]` to include 16× and 32×~~ **Done 2026-07-07** — 16× added (`scripts/rocket.gd:152`); 32× deliberately not added (Jason's call). Stepping through [> now goes 1× → 2× → 4× → 8× → 16×; [< reverses.
...
- ~~**Pause menu** — Esc during gameplay should pause + show a small panel with Continue and Main Menu options.~~ **Done 2026-07-07** — `scripts/pause_menu.gd` + `PauseMenu` (CanvasLayer) node in `level.tscn`. Esc pauses (sets `get_tree().paused = true`), shows the panel; Continue or Esc-while-paused unpauses; Main Menu unpauses + returns to main menu. Uses `PROCESS_MODE_ALWAYS` so the menu stays interactive while the rest of the game tree is paused; layer 100 draws over the HUD.
- **Trajectory line at escape velocity** — **Done 2026-07-08 (uncommitted)**. ELLIPSE-mode closed-form math now routes to an analytical **forward-direction hyperbola leg** when specific orbital energy ≥ 0, instead of collapsing to a single point. Implementation in `scripts/trajectoryline_2d.gd`: new `_is_bound(r, v, mu)` gate, new `_hyperbola_leg(r, v, mu, offset)` that sweeps from the rocket's current true anomaly toward the forward-going asymptote (ν → ±acos(-1/e) in the direction of the angular-momentum sign), clipped by `max_leg_radius` @export (default 1500) so the curve doesn't run off-screen. New `hyperbolic_color` @export (orange) so the player can see they're escaping. PROJECTED mode was already correct via forward-sim — fix is ELLIPSE-only. Mental-traced escape case (rocket at (500, 0), v=(0, 2.5), sun M=1000): ε=1.125, e=2.125, θ_inf≈118°, sweep bends up-right toward the asymptote and clips at 1500 px around θ≈89°.
...
- **Crash sound effect** *(queued 2026-07-08)* — currently the only "you crashed" feedback is a visual flash + the lose-screen transition. Add a crash SFX to `scripts/audio_manager.gd` (route to SFX bus), wire it into `rocket.gd`'s crash branch (sibling to the existing `play_astronaut_pickup` / `play_fuel_pickup` oneshots). Pick a low-frequency impact + short noise burst; should land hard enough to read as failure but not clip against the thruster loop. Audio asset TBD (need a `crash.mp3` under `res://assets/sound/`).
- **Custom Astro-Rescue title image** *(queued 2026-07-08)* — the main menu currently shows the title as a plain `Label` (`scenes/main_menu.tscn: TitleLabel`). Replace with a custom-drawn title image (logo-style or hand-drawn type) for visual identity. Drop the asset at `res://assets/textures/title.png` and swap the Label for a `TextureRect` with the new resource. Sized to fit roughly the same vertical region as the current 48-pt label. Companion consideration: a separate "logo lockup" if we ever want a splash screen.
- **Background animation in main menu** *(queued 2026-07-08)* — the main menu is currently a flat `ColorRect` backdrop. Replace (or layer on top of) with a non-interactive level running in the background — a level loaded from JSON, with no rocket / no controls, just bodies orbiting under closed-form math and the trajectory lines drawn passively. Implementation sketch: a `SubViewport` overlay (or a parallel scene tree) that instances the level without the rocket or input handlers; the existing `scripts/level_loader.gd` already supports this if we add a `background_only: bool` flag that skips the rocket. Camera slowly pans / zooms on a fixed cycle so the background doesn't feel static. Same `default_bus_layout.tres` Music bus plays the menu music underneath — no extra audio work needed.
- ~~**Audio / volume settings** *(queued 2026-07-08)*~~ **Done 2026-07-08** (`c6c0980`). All three sub-pieces done: (a) `default_bus_layout.tres` defines Master + Music + SFX buses; `AudioManager` routes `_music_player → Music`, thruster loop + oneshots → SFX. (b) `scripts/settings_menu.gd` is a `CanvasLayer` (process_mode = ALWAYS, layer 110) with Audio tab + Visual/Gameplay placeholders, accessible from main menu (Settings button) and pause menu (Settings button between Continue and Main Menu). Each channel row has slider (0..100 → -40..0 dB linear) + live dB readout + mute checkbox, wired to `AudioServer.set_bus_volume_db` / `set_bus_mute`. (c) `AudioSettings` autoload persists to `user://settings.cfg`; loaded in `_ready` and reapplied to AudioServer so persisted values survive restarts. Each setter pushes + persists immediately (no in-memory only state).
...
- **Project root:** `rocketman/astrorescue-main/rocketman/`
- **Git:** 1 commit ahead of origin/main — `c6c0980 feat(settings): audio volume sliders + settings menu shell`. Pre-existing commits (43 total from prior session) were pushed last night; origin/main moved from `2625cea` to `5877ed5`. Working tree (uncommitted, un-pushed): `scripts/trajectoryline_2d.gd` (escape-line fix), `Astro-Rescue.md` (these updates), `AstroRescueV01.exe` and `export_presets.cfg` (Godot drive-bys — existing pattern, leave alone).
